...
public:
    ...
    bool load(const std::string &key, const std::string &path, uint32_t scale);

private:
    boden::graphic::texture_manager_t *_texture_manager;
    std::unordered_map<std::string, boden::graphic::texture_id_t> _map;

    bool load_png(const std::string &key, const std::string &path, uint32_t scale);
    bool load_svg(const std::string &key, const std::string &path, uint32_t scale);
};
